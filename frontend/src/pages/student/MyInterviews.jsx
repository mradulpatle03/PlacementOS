...
function InterviewRow({ interview }) {
  ...
  const isUpcoming =
    !isPast(new Date(interview.scheduledAt)) &&
        ...
        <div className="space-y-2 min-w-0 flex-1">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {result && (
              ...
              >
                ...
                {result.label}
                {interview.ratingOutOf10 != null &&
                  ` · ${interview.ratingOutOf10}/10`}
              </span>
            )}
          </div>
          ...
          {interview.panel?.length > 0 && (
            <p className="text-xs text-muted-foreground">
              Panel:{" "}
              {interview.panel
                .map((p) => p.name || p.email || "Interviewer")
                .join(", ")}
            </p>
          )}
...
function BookSlotDialog({ open, onClose, onBooked }) {
  ...
  const { data: appsData, isLoading: appsLoading } = useQuery({
    ...
    queryFn: () =>
      getMyApplications({ limit: 50 }).then(
  ...
  const ROUND_REQUIRES = {
    interview_1: ["oa", "interview_1"],
    interview_2: ["interview_1", "interview_2"],
    hr: ["interview_2", "hr"],
  };

  const selectedApp = interviewApps.find((a) => a._id === appId);
  const slots = slotsData || [];
  const noSlotsReason = slotsData?.reason;
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex gap-2">
                {[
                  ...
                  const disabled =
                    selectedApp &&
                    !ROUND_REQUIRES[r.value].includes(selectedApp.status);
                  return (
                    <button
                      key={r.value}
                      disabled={disabled}
                      onClick={() => !disabled && setRound(r.value)}
                      className={cn(
            ...
            <div className="space-y-1.5">
              ...
              {slotsLoading ? (
                ...
                <div className="rounded-lg border border-dashed px-4 py-6 text-center">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {noSlotsReason || "No available slots for this round yet."}
                  </p>
                  {!noSlotsReason && (
...
export default function MyInterviews() {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: () =>
      interviewAPI.getMyInterviews().then((r) => r.data.data.interviews),
  });
  ...
  const filtered = interviews.filter((iv) => {
    ...
    if (filter === "upcoming")
      return !isPast(new Date(iv.scheduledAt)) && iv.status !== "cancelled";
    if (filter === "past")
      return isPast(new Date(iv.scheduledAt)) || iv.status === "completed";
