...
function CreateSlotDialog({ driveId, open, onClose, onCreated }) {
  ...
  const mutation = useMutation({
    ...
    onError: (err) => {
      ...
      toast.error(
        Array.isArray(msg) ? msg.join(", ") : msg || "Failed to create slot",
      );
    },
  });
  ...
  const mutation = useMutation({
    mutationFn: () =>
      interviewAPI.recordResult(interview._id, {
        ...form,
        ratingOutOf10:
          form.ratingOutOf10 !== "" ? Number(form.ratingOutOf10) : null,
      }),
    onSuccess: () => {
        ...
        <div className="space-y-3 py-2">
            ...
            <div className="flex gap-2">
              {[
                ...
                },
                {
                  value: "no_show",
                  label: "— No Show",
                  cls: "bg-amber-400 border-amber-400 text-white",
                },
              ].map((r) => (
                <button
                  key={r.value}
          ...
          <div className="space-y-1">
            ...
            <Input
              ...
              onChange={(e) =>
                setForm((p) => ({ ...p, ratingOutOf10: e.target.value }))
              }
              placeholder="e.g. 7.5"
              className="text-sm"
            />
...
export default function InterviewManager() {
  return (
    <ErrorBoundary
      fallback={
        <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-4">
          <p className="font-semibold">The interview manager crashed.</p>
    ...
    >
      <InterviewManagerInner />
    </ErrorBoundary>
  );
}

// ── Main page ─────────────────────────────────────────────────
function InterviewManagerInner() {
  ...
  const { data: interviewsData, isLoading: interviewsLoading } = useQuery({
    ...
    queryFn: () =>
      interviewAPI
        .getByDrive(driveId, roundFilter ? { round: roundFilter } : {})
  ...
  const deleteMutation = useMutation({
    ...
    onError: (err) =>
      toast.error(err?.response?.data?.message || "Cannot remove slot"),
  });
