...
const applyToDrive = async (req, res, next) => {
  try {
    ...
    // 2. drive must be open for applications
    if (drive.status !== 'open') {
      return next(new AppError(`Applications are not open for this drive (status: ${drive.status})`, 400));
    }
    ...
    if (!resume) return next(new AppError('Resume not found or does not belong to you', 404));

    // 6. prevent duplicate application (unique index will also catch this)
    ...
    if (existing) {
      if (existing.status === 'withdrawn') {
        return next(new AppError('You have withdrawn from this drive and cannot re-apply', 400));
      }
...
const withdrawApplication = async (req, res, next) => {
  try {
    ...
    if (nonWithdrawableStatuses.includes(application.status)) {
      return next(
        new AppError(`Cannot withdraw an application with status '${application.status}'`, 400)
      );
    }
...
const getApplicationsByDrive = async (req, res, next) => {
  try {
    ...
    const { status, branch, sort = '-appliedAt', page = 1, limit = 20 } = req.query;
    ...
    let query = Application.find(filter)
      .populate({
        path: 'student',
        select: 'branch cgpa backlogs graduationYear rollNumber placementStatus',
...
module.exports = { applyToDrive, withdrawApplication, getMyApplications, getApplicationsByDrive };